---
name: ks-assess-answers
description: Assess student answers captured by the diagnostic shell script. Reads raw answers and the full curriculum, grades each response, and writes results to the student's progress file. Infers course from topic slug. Usage: /ks-assess-answers [student-name] [topic-slug] [date]
---
...
```
/ks-assess-answers [student-name] [topic-slug] [date] ["optional tutor comment"]
```

**Examples:**
- `/ks-assess-answers` (lists all unassessed responses across all students and courses)
- `/ks-assess-answers Freya` (lists unassessed responses for Freya)
- `/ks-assess-answers Freya forces 2026-02-02`
- `/ks-assess-answers Freya christianity-beliefs 2026-02-09`
- `/ks-assess-answers Freya energy` (uses most recent file if date omitted)
- `/ks-assess-answers Freya forces 2026-02-08 "seemed hesitant on calculations"`

## Workflow

### Step 0: Resolve Parameters and Course

**If no topic is provided** (just `/ks-assess-answers` or `/ks-assess-answers [student]`):
...
Run: `/ks-assess-answers [student] [topic] [date]` to assess one.
```
